...
const SHEETS = {
  ...
  // One row per stock item (product) so each Pokémon/type held on an account shows up on its own
  // line; accounts with no stock still get a single "-" row so the account itself isn't dropped.
  accounts: { title: "Accounts", headers: ["ชื่อไอดี", "ชื่อสินค้า (Pokémon)", "ชนิด", "จำนวนคงเหลือ", "แจ้งเตือนเมื่อเหลือ ≤", "ลงทุนสะสม (ไอดี)", "รายรับสะสม (ไอดี)"] },
...
// One account -> one row per stock item (product), e.g.:
//   MyAcc1   Rayquaza   Shiny        3   2   1500   4200
//   MyAcc1   Mewtwo     ปกติ         5   2   1500   4200
// ลงทุนสะสม/รายรับสะสม are account-level totals repeated on every row so each
// row stays self-contained for filtering/summing in Sheets. Accounts with no
// stock still get one row (with "-" in the product columns) so they aren't dropped.
function accountRows(a, data) {
  ...
  const stock = a.stock || [];
  if (!stock.length) {
    return [[a.name, "-", "-", 0, "", invested, income]];
  }
  return stock.map((s) => [
    a.name,
    s.name,
...
export async function syncAll({ token, spreadsheetId, folderId, data, onStatus }) {
  ...
  const accountSheetRows = data.gameAccounts.flatMap((a) => accountRows(a, { ...data, orders }));
  const financeRows = [
  ...
  const writes = [
    ...
    { range: `${SHEETS.accounts.title}!A2`, values: accountSheetRows },
