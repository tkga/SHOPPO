...
const SHEETS = {
  ...
  stock: { title: "Stock", headers: ["ชื่อไอดี", "ชื่อสินค้า (Pokémon)", "ชนิด", "จำนวนคงเหลือ", "แจ้งเตือนเมื่อเหลือ ≤"] },
...
function driveImageFormula(fileId) {
  ...
  return `=IMAGE("https://drive.google.com/uc?export=view&id=${fileId}", 4, 76, 76)`;
}
...
// Full sync: uploads any pending order photos, then overwrites all five
...
export async function syncAll({ token, spreadsheetId, folderId, data, onStatus }) {
  ...
  const accountSummaryRows = data.gameAccounts.map((a) => accountSummaryRow(a, { ...data, orders }));
  const stockSheetRows = data.gameAccounts.flatMap((a) => stockRows(a));
  const financeRows = [
