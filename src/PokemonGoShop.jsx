...
import {
  ...
  Target, Move, Image as ImageIcon
} from "lucide-react";
import {
...
// Order code from a running counter: a0001..a9999, then b0001..b9999, then c0001.. and so on.
function orderCodeFromCounter(n) {
...
// Autocomplete sources: unique Pokémon names already used anywhere (stock items + past orders),
// and unique existing account (ไอดี) names — used to power <datalist> suggestions on free-text
// name inputs so the shop owner sees existing names instead of retyping/mistyping them.
function uniquePokemonNames(data) {
...
function existingAccountNames(data) {
  const seen = new Map();
...
// ---- trash (soft delete) ----
// move a deleted record here instead of dropping it, so it can be restored later.
// `meta` carries anything needed to put it back where it came from (e.g. which
// account a stock item belonged to).
function pushTrash(trash, type, payload, meta = {}) {
...
// ---- stock movement log ----
// every quantity change gets one line here: what changed, why, and the
// resulting quantity — so "why is this number what it is" is always answerable.
function pushStockMovement(stockMovements, entry) {
  ...
  return (
    <div className="pgs-root">
      ...
      <div className="pgs-scroll">
        ...
        {tab === "hire" && <HireTab data={data} custName={custName} accName={accName} openEdit={(o) => setModal({ type: "order", mode: "edit", item: o })} onQuickUse={quickUseHire} onQuickHireStatus={quickSetHireStatus} onQuickCancel={promptCancelOrder} />}
      </div>
      {moreOpen && (
        <MoreSheet
...
function ProofImagePicker({ value, onChange, alt = "รูปภาพกิจกรรม", addLabel = "แนบรูปภาพ" }) {
...
function AccountModal({ mode, item, data, onClose, onSave }) {
...
function StockModal({ mode, item, data, onClose, onSave, onDelete }) {
...
// compact log of quantity changes for one stock item — newest first, capped to the
// most recent 20 so it doesn't take over the modal; explains "why is this number what it is".
function StockMovementHistory({ data, stockItemId }) {
...
// =================== HIRE (ตีบอส / เชิญตี) ===================
function HireTab({ data, custName, accName, openEdit, onQuickUse, onQuickHireStatus, onQuickCancel }) {
...
function TrashModal({ data, onClose, onRestore, onPurge }) {
...
// =================== SETTINGS ===================
function SettingsTab({ data, setData, onBackup, onRestore, onExportExcel, onExportPDF, showToast, back, googleSyncing, googleStatus, onConnectGoogle, onSyncNow, onDisconnectGoogle, openTrash }) {
  const fileRef = useRef(null);
